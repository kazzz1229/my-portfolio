...
session_save_path("/var/tmp/freemarket");
...
{
    //DBへの接続準備
    $dsn = 'mysql:dbname=freemarket;host=training-db.cf5r6kpw2ib3.ap-northeast-1.rds.amazonaws.com;charset=utf8';
    $user = 'admin';
    $password = '88564893bb';
    $options = array(
...
function getProductOne($p_id)
{
    ...
    try {
        ...
        $dbh = dbConnect();
        // SQL文作成
        $sql = 'SELECT p.id , p.name , p.comment, p.price, p.pic1, p.pic2, p.pic3, p.user_id, p.create_date, p.update_time, c.name AS category 
    ...
    try {
        ...
        if (!empty($rst)) {
            foreach ($rst as $key => $val) {
                // debug('$rst:'.print_r($rst, true));  $rst は掲示板のデータ全て(多次元配列)
                // debug('$key:'.print_r($key, true));  $key は 0 から始まる添え字
                // debug('$val:'.print_r($val, true));  $val は 掲示板のデータが一つずつ(配列)
                // SQL文作成
